...
export default function KitchenDisplay() {
  ...
  const kitchenOrders = orders
    .filter((o) => o.type !== 'drink' && o.status !== 'served')
    .sort((a, b) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className={`min-h-screen bg-[#0A0A0A] flex flex-col transition-colors duration-300 ${flash ? 'bg-orange-950/30' : ''}`}
      style={{
        backgroundImage: 'radial-gradient(ellipse at 50% 0%, rgba(249,115,22,0.06) 0%, transparent 60%)',
      }}
    >
      {/* Header */}
        ...
        <div className="flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-white font-bold text-lg">Kitchen</h1>
                <p className="text-[#9CA3AF] text-xs">{kitchenOrders.length} active orders</p>
              </div>
            </div>
          </div>
          {newCount > 0 && (
            ...
            >
              ...
              <span className="text-red-400 font-bold text-sm">{newCount} new</span>
            </motion.div>
          )}
        </div>
      </div>
        ...
        <AnimatePresence mode="popLayout">
          {kitchenOrders.length === 0 ? (
            ...
            >
              ...
              <p className="text-sm mt-1">No active orders</p>
            </motion.div>
          ) : (
